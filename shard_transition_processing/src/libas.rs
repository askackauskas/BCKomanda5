...
pub fn proccess_shard_transitions<C: Config>(state:&mut BeaconState<C>, shard_transitions:Vec<ShardTransition<C>>,attestations: Vec<Attestation<C>>){
...
pub fn verify_empty_shard_transitions<C: Config>(state:&mut BeaconState<C>,  shard_transitions:Vec<ShardTransition<C>>)->bool{
    ...
    if (state.shard_states[shard as usize].slot != compute_previous_slot(state.slot) &&  shard_transitions[shard as usize] != ShardTransition{..Default::default()}) {return false}
    }
     true
}

fn proccess_crosslinks<C: Config>(state:&mut BeaconState<C>,shard_transitions:Vec<ShardTransition<C>>,attestations: Vec<Attestation<C>>)-> Result<()> {
