...
use crate::{
    utils::{decode_variable_sized_items, encode_offset},
    SszDecode, SszDecodeError, SszEncode, BYTES_PER_LENGTH_OFFSET,
};
...
mod test {
    ...
    mod serialize {
        ...
        #[test]
        fn variable() {
            let vec: FixedVector<VariableList<u8, U4>, U3> =
                FixedVector::from(vec![vec![1, 2].into(), vec![].into(), vec![3].into()]);
            assert_eq!(
                vec.as_ssz_bytes(),
                vec![12, 0, 0, 0, 14, 0, 0, 0, 14, 0, 0, 0, 1, 2, 3]
            );

            let vec: FixedVector<VariableList<u8, U4>, U5> =
                FixedVector::from(vec![vec![1, 2].into(), vec![].into(), vec![3, 4, 5].into()]);
            assert_eq!(
                vec.as_ssz_bytes(),
                vec![
    ...
    mod deserialize {
        ...
        #[test]
        fn variable() {
            let vec = <FixedVector<VariableList<u8, U4>, U3> as SszDecode>::from_ssz_bytes(&[
            ...
            assert_eq!(
                vec.to_vec(),
                vec![vec![1, 2].into(), vec![].into(), vec![3].into()]
            );

            let vec = <FixedVector<VariableList<u8, U4>, U5> as SszDecode>::from_ssz_bytes(&[
                20, 0, 0, 0, 22, 0, 0, 0, 22, 0, 0, 0, 25, 0, 0, 0, 25, 0, 0, 0, 1, 2, 3, 4, 5,
            ])
            .expect("Test");
            assert_eq!(
        ...
        mod errors {
            ...
            fn wrong_size() {
                ...
                let result =
                    <FixedVector<VariableList<u8, U4>, U6> as SszDecode>::from_ssz_bytes(&[
                        12, 0, 0, 0, 14, 0, 0, 0, 14, 0, 0, 0, 1, 2, 3,
                    ]);
                assert!(result.is_err());
            }
