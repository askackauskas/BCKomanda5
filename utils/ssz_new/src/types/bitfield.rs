...
impl<N: Unsigned + Clone> SszEncode for Bitfield<Variable<N>> {
...
impl<N: Unsigned + Clone> SszDecode for Bitfield<Variable<N>> {
...
impl<N: Unsigned + Clone> SszEncode for Bitfield<Fixed<N>> {
    fn as_ssz_bytes(&self) -> Vec<u8> {
        self.clone().into_bytes()
    }
...
impl<N: Unsigned + Clone> SszDecode for Bitfield<Fixed<N>> {
    fn from_ssz_bytes(bytes: &[u8]) -> Result<Self, SszDecodeError> {
        Self::from_bytes(bytes.to_vec()).map_err(|e| {
...
mod tests {
    ...
    mod bitlist {
        use typenum::{U0, U1, U1024, U16, U8};

        use super::*;

        type BitList0 = Bitfield<Variable<U0>>;
        type BitList1 = Bitfield<Variable<U1>>;
        type BitList8 = Bitfield<Variable<U8>>;
        type BitList16 = Bitfield<Variable<U16>>;
        type BitList1024 = Bitfield<Variable<U1024>>;
